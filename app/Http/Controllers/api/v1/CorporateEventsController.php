...
class CorporateEventsController extends APIController {
        ...
        try {
            ...
            if(Auth::guard('corporate')->check()){
                ...
                $fetch_record = Events::select("*")
                                ...
                                ->where('event_corporate_id', $id);
            } else {
                $fetch_record = Events::select("*")
                                ->with(['images', 'addedByCor', 'event_members.member'])
                                ->withCount('event_members')
                                ->where('event_status', 1)
                                //->join('event_members', 'event_id', 'em_event_id')
                                ->where('event_owner_id', null);
            }
    ...
    public function get_all_events_by_group_id(Request $request) {
        try {
            ...
            $fetch_record = Events::select("*")
                    ->with(['images', 'addedByCor', 'event_members', 'event_members.corporatemember'])
                    ->withCount('event_members')
                    ->where('event_status', 1)
    ...
    public function create_event(Request $request) {
        try {
            ...
            $find_record->event_corporate_id = Auth::guard('corporate')->user()->u_id;
            ...
            if (!empty($find_record)) {
                ...
                if($request->event_participants != 'Public'){

                    if ($request->event_members != null) {
                ...
                return $this->respondResult($find_record, "Event saved successfully");
            } else {
    ...
    public function delete_event(Request $request) {
        try {
            ...
            if ($find_record->event_corporate_id != $id) {
    ...
    public function edit_event(Request $request) {
        try {
            ...
            if (!empty($find_record)) {
                ...
                return $this->respondResult("", "Event details updated successfully");
            } else {
    ...
    public function get_event_details(Request $request) {
        try {
            ...
            $value = Events::with(['images', 'event_members', 'event_members.member'])
                    ->select("*")
                    ->join('users', 'event_corporate_id', 'u_id')
            ...
            if (!empty($value)) {
                ...
                if (!empty($id)) {
                    ...
                    WHERE
                        (`ufr_user_id` = ' . $value->event_corporate_id . ' AND `ufr_invited_user_id` = ' . $id . ')
                        OR
                        (`ufr_user_id` = ' . $id . ' AND `ufr_invited_user_id` = ' . $value->event_corporate_id . ')
                    AND `u_status` != 9
                    AND `ufr_status` != 9
    ...
    public function all_events_corporate(Request $request) {
        try {
    ...
    public function get_corporate_events(Request $request,$id) {
        try {
    ...
    public function event_details($id){
        try {
            ...
            $fetch_record = Events::select("*")
                            ->with(['images', 'addedBy', 'addedByCor', 'event_members.member'])
                            ->withCount('event_members')
                            ->where('event_status', 1)
                            //->join('event_members', 'event_id', 'em_event_id')
            ...
            if ($fetch_record) {
                $response["result"] = $fetch_record;
                $response["message"] = "Events found successfully.";
            } else {
