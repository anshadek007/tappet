...
use ResetsApiPasswords;
    ...
    public function showResetForm(\Illuminate\Http\Request $request, $token = null) {
        ...
        return view('api.passwords.reset')->with(['token' => $token, 'email' => $request->email, 'password_resets' => $password_resets]);
    }
...
//        $this->guard()->login($user);
    ...
    public function reset(Request $request) {
        ...
        $get_user = User::select("*")->where("u_email", trim($request->get('u_email')))->first();
        if ($get_user) {
            $user = User::find($get_user->u_id);
            $password = $request->get('u_password');
            $user->u_password = \Hash::make($password);
            $user->save();

            \DB::table('password_resets')->where('email', $request->get('u_email'))->delete();
