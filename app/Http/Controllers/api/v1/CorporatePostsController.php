...
class CorporatePostsController extends APIController {
    ...
    public function get_all_posts(Request $request) {
        try {
            ...
            $fetch_record = Posts::select('posts.*', "u_id", "u_first_name", "u_last_name", "u_image",
                  ...
                  WHERE (`ufr_user_id` = ".$id." AND `ufr_invited_user_id` = post_corporate_id) OR (`ufr_user_id` = post_corporate_id AND `ufr_invited_user_id` = ".$id.")
                  LIMIT 1) as friend"))
                    ->join('users', 'post_corporate_id', 'u_id')
                ...
                foreach ($fetch_record as $value) {
                    ...
                    if (!empty($value->post_group_id)) {
                        ...
                        $check_group_status = GroupMembers::where('gm_group_id', $value->post_group_id)
                                ->where('gm_user_id', $value->post_corporate_id)
                                ->where('gm_status', 1)
                                ->count();
...
//                'post_image.max' => "The post file may not be greater than ".$max_file_size."MB."
            ...
            $find_record->post_corporate_id = $id;
...
//            dd($value);
    ...
    public function delete_post(Request $request) {
        try {
            ...
            if ($find_record->post_corporate_id != $id) {
        ...
        try {
            ...
            if (!empty($find_record)) {
                ...
                if ($favourite == 1) {
                    ...
                    $this->send_post_like_push($find_record->post_corporate_id, $id, $n_message, 5, 2, $user, $request->post_id);
    ...
    public function add_comment(Request $request) {
        try {
            ...
            if (!empty($find_record)) {
                ...
                if(!empty($find_record->post_corporate_id) && $find_record->post_corporate_id !=$id){
                    $n_message = $user->first_name . ' ' . $user->last_name . ' commented on a post you shared.';
                    $this->send_post_comment_push($find_record->post_corporate_id, $id, $n_message, 6, 2, $user, $request->post_id);
                }
            }
    ...
    public function get_all_posts_corporate(Request $request) {
        try {
            ...
            // \DB::enableQueryLog();
            $fetch_record = Posts::join('business_user', 'post_corporate_id', 'u_id')
                    ->with(['post_images', 'post_comments', 'post_comments.corporateUser'])
                    ->select('posts.*', "u_id", "u_first_name", "u_last_name", "u_image" ,"u_corporation_name")
                    ->withCount('post_is_liked')
                    ->withCount('post_likes')
                    ->leftJoin('user_friends', function ($join) {
                        $join->on('u_id', '=', 'ufr_invited_user_id')
            ...
            if (count($fetch_record) > 0) {
                foreach ($fetch_record as $value) {
                    ...
                    unset($value->event);
                    unset($value->post_is_liked_count);
                    $value->u_image = url('/public/uploads/busUser/'. $value->u_id.'/'. $value->u_image);
                    $fetch_record_list[] = $value;
                }
                $message = "Posts found successfully.";
            } else {
